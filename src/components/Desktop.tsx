...
export const Desktop = ({ city, info }: CityWeatherInterface) => {
  ...
  const determineNext5Days = (): string[] => {
    ...
    for (let i = 0; i < 4; i++) {
      ...
      next5Days.push(days[dateTime.getDay()]);
    }
  ...
  const determineTimestamps = (day: number, list: any[]): any[] => {
    ...
    for (const e of list) {
      if (
        parseInt(`${e["dt_txt"][8]}${e["dt_txt"][9]}`) === dateTime.getDate()
      ) {
        timestamps.push({
  ...
  const timestamps = info?.list
    ? adaptToWidth(determineTimestamps(day, info?.list))
    : [];
    ...
    <>
      <div className="w-screen flex justify-between" style={{ height: "65%" }}>
        <div className="text-white pt-8 pl-8">
          ...
          <div className="text-xl my-1 sm:my-0">{city}</div>
